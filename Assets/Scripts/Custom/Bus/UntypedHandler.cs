...
namespace Custom.Bus {
	sealed class UntypedHandler : HandlerBase {
		...
		public void Fire() {
			...
			CleanUp();
			if ( AllFireLogs ) {
				Debug.LogFormat("[{0}] fired (Listeners: {1})", nameof(UntypedHandler), Watchers.Count);
			}
		}
		...
		public override bool FixWatchers() {
			...
			if ( (count > 0) && LogsEnabled ) {
				Debug.LogErrorFormat("{0} destroyed scripts subscribed to event {1}.", count,
					typeof(UntypedHandler));
			}
			return (count == 0);
		}
	}
